...
! -----------------------------------------------------------------------
   ...
   real(kind=8) :: tnorm, abs, s, fact, t
...
subroutine ilutp(n, a, ja, ia, lfil, droptol, permtol, mbloc, alu, jlu, ju, iwk, w, jw, iperm, ierr)
   integer, intent(In) :: n, lfil, iwk, mbloc
   integer, dimension(*), intent(Out) :: ja, jlu
   integer, dimension(n + 1), intent(In) :: ia
   integer, dimension(n), intent(InOut) :: ju
   integer, dimension(2*n), intent(Out) :: jw
   integer, dimension(2*n), intent(Out) :: iperm
   integer, intent(Out) :: ierr

   real(kind=8), dimension(*), intent(In) :: a
   real(kind=8), dimension(*), intent(InOut) :: alu
   real(kind=8), dimension(n + 1), intent(Out) :: w
   real(kind=8), intent(In) :: droptol, permtol
! -----------------------------------------------------------------------
!      implicit none
! ----------------------------------------------------------------------*
...
!
   ...
   real(kind=8) :: s, tmp, tnorm, xmax, xmax0, fact, abs, t
...
! -----------------------------------------------------------------------
   ...
   real(kind=8), dimension(*), intent(In) :: a
   real(kind=8), dimension(*), intent(InOut) :: alu
   real(kind=8), dimension(2*n), intent(Out) :: w
   real(kind=8), intent(In) :: tol, alph
   integer, dimension(*), intent(In) :: ja
   integer, dimension(n + 1), intent(In) :: ia
   integer, dimension(*), intent(Out) :: jlu
   integer, dimension(n), intent(InOut) :: ju
...
! -----------------------------------------------------------------------
!      locals
   integer :: ju0, k, j1, j2, j, i, ii, lenl, lenu, jj, jrow, jpos, len
   real(kind=8) :: tnorm, t, abs, s, fact, dropsum
! -----------------------------------------------------------------------
!      initialize ju0 (points to next element to be added to alu,jlu)
!      and pointer array.
! -----------------------------------------------------------------------
...
! -----------------------------------------------------------------------
   integer, intent(In) :: n, mbloc
   integer, dimension(*), intent(Out) :: ja, jlu
   integer, dimension(n + 1), intent(In) :: ia
   integer, dimension(n), intent(InOut) :: ju
   integer, dimension(2*n), intent(Out) :: jw, iperm
   ...
   real(kind=8), dimension(*), intent(InOut) :: alu
   real(kind=8), dimension(2*n), intent(Out) :: w
   real(kind=8), intent(In) :: alph, droptol, permtol
! ----------------------------------------------------------------------*
...
!
   integer :: k, i, j, jrow, ju0, ii, j1, j2, jpos, len, imax, lenu, lenl, jj, icut
   real(kind=8) :: s, tmp, tnorm, xmax, xmax0, fact, abs, t, dropsum
! -----------------------------------------------------------------------
!      initialize ju0 (points to next element to be added to alu,jlu)
!      and pointer array.
! -----------------------------------------------------------------------
...
subroutine iluk(n, a, ja, ia, lfil, alu, jlu, ju, levs, iwk, w, jw, ierr)
   ...
   integer, dimension(*), intent(Out) :: jlu
   integer, dimension(n), intent(InOut) :: ju
   integer, dimension(*), intent(InOut) :: levs
   integer, dimension(3*n), intent(Out) :: jw
   integer, intent(Out) :: ierr
! ----------------------------------------------------------------------*
...
! END new declarations
   real(kind=8), dimension(*), intent(In) :: a
   real(kind=8), dimension(*), intent(Out) :: alu
   integer, dimension(*), intent(In) :: ja, ia
   integer, dimension(*), intent(Out) :: ju, jlu, iw
...
! -----------------------------------------------------------------------
   ...
   real(kind=8) :: eps1, ro, dnrm2, ddot, gam, epsmac
   integer :: its, j, i, i1, k, k1, ii, jj

   integer, intent(In) :: n, im, maxits
   integer :: iout
   integer, intent(Out) :: ierr
   integer, dimension(*), intent(In) :: ja, jlu
   integer, dimension(n + 1), intent(In) :: ia
   integer, dimension(n), intent(In) :: ju
   real(kind=8), dimension(n, *), intent(InOut) :: vv
   real(kind=8), dimension(n), intent(InOut) :: rhs, sol
   real(kind=8), dimension(*), intent(In) :: aa, alu
...
! ----------------------------------------------------------------------*
   ...
   real(kind=8), dimension(kmax + 1, kmax) :: hh
   real(kind=8), dimension(kmax) :: c, s
...
subroutine lusol(n, y, x, alu, jlu, ju)
   ...
   real(kind=8), dimension(*), intent(In) :: alu
   integer, intent(In) :: n
...
subroutine lutsol(n, y, x, alu, jlu, ju)
   ...
   real(kind=8), dimension(n), intent(Out) :: x
   real(kind=8), dimension(n), intent(In) :: y, alu
   integer, intent(In) :: n
   integer, dimension(*), intent(In) :: jlu, ju
! -----------------------------------------------------------------------
!
